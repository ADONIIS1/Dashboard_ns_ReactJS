...
const Degree = () => {
  ...
  const handleSubmit = async (event,_id) => {
    ...
    if(_id == undefined){
        setDegree(degree);
      const data = await degreeService.create(degree).then(res => {
        return res;
      }).catch((err) => {
        return err;
      });    
        ...
        setDegree(degree);
      await degreeService.update(_id,degree).then(res => {
        return res;
      }).catch((err) => {
        return err;
      });
        ...
        <table className="w-full table-auto">
          <thead>
            <tr>
              ...
              <th className="px-4 py-2" onClick={() => setSorting({ criteria: 'specialized', direction: sorting.direction === 'asc' ? 'desc' : 'asc' })}>Specialized</th>
              <th className="px-4 py-2" onClick={() => setSorting({ criteria: 'majors', direction: sorting.direction === 'asc' ? 'desc' : 'asc' })}>Majors</th>
              ...
              <tr key={index} className={index % 2 === 0 ? 'bg-gray-100' : ''}>
                ...
                <td className="border px-4 py-2">
                  <div className="flex space-x-2">
                    <button className="text-blue-600 hover:text-blue-800"><HiPencilAlt size={20} onClick={() => handleEdit(post._id, post.name, post.specialized, post.majors)}/></button>
