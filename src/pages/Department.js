...
const BlogManager = () => {
  ...
  const handleSubmit = async (event,_id) => {
    ...
    if(_id == undefined){
      ...
      const data = await departmentService.create(department).then(res => {
        return res;
      }).catch((err) => {
        return err;
      });    
      ...
      setDepartment(department);
      await departmentService.update(_id,department).then(res => {
        return res;
      }).catch((err) => {
        return err;
      });
      ...
      <div className="w-full overflow-x-auto" style={{ paddingTop: '20px' }}>
        ...
        <thead>
          <tr>
            ...
            <th className="px-4 py-2" onClick={() => setSorting({ criteria: 'address', direction: sorting.direction === 'asc' ? 'desc' : 'asc' })}>Address</th>
        ...
        <tbody>
          {jsonData.sort(getSortingFunction(sorting.criteria, sorting.direction)).map((post, index) => (
            <tr key={index} className={index % 2 === 0 ? 'bg-gray-100' : ''}>
              ...
              <button className="bg-blue-500 hover:bg-red-300 text-white font-bold py-1 px-2 rounded-full" onClick={handleButtonClick}>
                Sched
              </button>
              <button className="bg-blue-500 hover:bg-red-300 text-white font-bold py-1 px-2 rounded-full" onClick={handleButtonClick}>
                Emp
